...
export function renderDashboard() {
  ...
  if (predictions.length === 0) {
    tableRows = `<tr><td colspan="4" class="px-md py-lg text-center text-outline">Belum ada prediksi. Silakan ke menu Prediksi untuk memulai.</td></tr>`;
  } else {
    predictions.slice(0, 5).forEach(p => {
      ...
      tableRows += `
        <tr class="prediction-row hover:bg-surface-container-low transition-colors">
          <td class="px-md py-md font-mono-data text-outline">#${String(p.id).slice(-5)}</td>
          <td class="px-md py-md font-semibold">GTZ-5S</td>
          <td class="px-md py-md font-mono-data text-primary font-bold">${p.soh || '-'}%</td>
  ...
  <div class="space-y-lg page-enter">
    ...
    <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-md">
      <div class="spectrum-gradient-border p-md bg-white">
        <div class="flex flex-col gap-xs">
          <span class="text-label-caps font-label-caps text-outline uppercase">MAE (Error Absolut)</span>
      ...
      <div class="spectrum-gradient-border p-md bg-white">
        <div class="flex flex-col gap-xs">
          <span class="text-label-caps font-label-caps text-outline uppercase">MAE SOH (Error Kesehatan)</span>
          <div class="flex items-baseline gap-xs">
            <span class="text-h2 font-h2 text-on-background">0.84</span>
      ...
      <div class="spectrum-gradient-border p-md bg-white">
        <div class="flex flex-col gap-xs">
          <span class="text-label-caps font-label-caps text-outline uppercase">Skor R² (Akurasi)</span>
          <div class="flex items-baseline gap-xs">
    ...
    <section class="bg-white rounded-xl border border-outline-variant p-lg shadow-sm">
      <div class="flex justify-between items-center mb-lg">
        <div>
          <h3 class="text-h3 font-h3 text-on-background">Tren Prediksi Sesi Ini</h3>
          <p class="text-body-md text-outline">Visualisasi real-time dari sisa umur (RUL) yang Anda prediksi</p>
        </div>
      </div>
      <div id="dashboard-chart" class="relative w-full h-80 flex items-center justify-center rounded-lg bg-surface-container-low/30 border border-surface-container overflow-hidden">
        ${!hasData ? `
          <div class="text-center space-y-sm">
            ...
            <p class="text-outline">Belum ada data untuk divisualisasikan</p>
          </div>
        ` : ''}
      </div>
    </section>
    ...
    <section class="grid grid-cols-1 lg:grid-cols-3 gap-lg">
      <div class="lg:col-span-2 bg-white rounded-xl border border-outline-variant overflow-hidden shadow-sm">
        <div class="p-md bg-surface-container-low flex justify-between items-center border-b border-outline-variant">
          <h3 class="text-body-lg font-h3 text-on-background">Prediksi Terbaru</h3>
          <button onclick="window.navigateTo('inputs')" class="text-primary font-label-caps flex items-center gap-xs">
            Buat Prediksi Baru <span class="material-symbols-outlined text-[16px]">chevron_right</span>
          </button>
        </div>
          ...
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="bg-surface-bright text-label-caps text-outline uppercase border-b border-outline-variant">
                ...
                <th class="px-md py-sm">RUL (Siklus)</th>
      ...
      <div class="bg-primary-container rounded-xl p-lg text-white flex flex-col justify-between shadow-lg relative overflow-hidden">
        <div class="z-10">
          <span class="text-label-caps opacity-90 uppercase block mb-sm">Penghitung Sesi</span>
          <span class="text-[48px] font-bold font-mono-data leading-none">${totalPredictions}</span>
          <p class="text-body-md opacity-80 mt-xs">Prediksi telah dilakukan</p>
        </div>
